...
export function SteeringEditor({ filename, onClose, onSaved }: SteeringEditorProps) {
  ...
  const loadContent = useCallback(async () => {
    ...
    setError(null)
    try {
      const result = await window.api.readAppSteeringFile(filename)
  ...
  const saveContent = async () => {
    ...
    setError(null)
    try {
      const result = await window.api.saveAppSteeringFile(filename, content)
      if (result.success) {
        setModified(false)
        onSaved?.()
      } else {
      ...
      <div className="relative bg-background rounded-lg shadow-xl w-[90vw] max-w-4xl h-[80vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b">
          <div className="flex items-center gap-2">
            ...
            {modified && (
              <span className="text-xs text-orange-500">● {isEn ? 'Modified' : '已修改'}</span>
            )}
          </div>
          <div className="flex items-center gap-2">
