...
async function getMacOSMachineId(): Promise<MachineIdResult> {
  try {
    ...
    // 检查其他应用的 machineid 文件
    const otherAppMachineIdPath = path.join(
      process.env.HOME || '',
      'Library/Application Support/ElectronSQLiteRest/machineid'
    )
    if (fs.existsSync(otherAppMachineIdPath)) {
      const otherAppId = fs.readFileSync(otherAppMachineIdPath, 'utf-8').trim()
      if (otherAppId && isValidMachineId(otherAppId)) {
        return { success: true, machineId: otherAppId }
      }
    }
...
async function setMacOSMachineId(newMachineId: string): Promise<MachineIdResult> {
  ...
  // 我们写入本应用的 override 文件，并同步到其他应用的 machineid 文件
  const overridePath = path.join(app.getPath('userData'), 'machine-id-override')
  const otherAppMachineIdPath = path.join(
    process.env.HOME || '',
    'Library/Application Support/ElectronSQLiteRest/machineid'
  ...
  try {
    ...
    // 同步到其他应用的 machineid 文件
    try {
      const otherAppDir = path.dirname(otherAppMachineIdPath)
      if (!fs.existsSync(otherAppDir)) {
        fs.mkdirSync(otherAppDir, { recursive: true })
      }
      fs.writeFileSync(otherAppMachineIdPath, newMachineId, 'utf-8')
      console.log('[MachineId] Synced to other app machineid:', otherAppMachineIdPath)
    } catch (syncError) {
      console.warn('[MachineId] Failed to sync to other app:', syncError)
      // 同步失败不影响主流程
    }
